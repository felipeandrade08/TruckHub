    ...
    try{
      ...
      if(companyId){
        ...
        if(since===null){
          ...
          rows=await sql`WITH incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l JOIN trip_settlement_completions sc ON sc.trip_id=l.trip_id AND sc.user_id=l.user_id WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT u.id,u.name,u.email,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM users u LEFT JOIN trips t ON t.user_id=u.id AND t.status='finished' AND COALESCE(t.finished_at,t.started_at)>=${since.toISOString()} AND EXISTS(SELECT 1 FROM incomes si WHERE si.trip_id=t.id) LEFT JOIN incomes i ON i.trip_id=t.id WHERE u.id=${u.id} GROUP BY u.id,u.name,u.email`;
        }
      }
