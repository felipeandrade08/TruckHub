...
export function registerDriverDashboardRoutes(app:any){
  ...
  app.get('/me/financial-summary',async c=>{
    ...
    const totals=await sql`SELECT COALESCE(SUM(s.driver_gross),0)::numeric revenue,COALESCE(SUM(s.driver_expenses),0)::numeric expenses,
      ...
      FROM company_trip_settlements s JOIN trip_settlement_completions sc ON sc.trip_id=s.trip_id AND sc.user_id=s.user_id JOIN trips t ON t.id=s.trip_id WHERE s.user_id=${u.id}`;
    ...
    const topTrips=await sql`SELECT t.id,t.origin,t.destination,t.cargo,t.started_at,t.distance_km,s.driver_gross revenue,s.driver_expenses expenses,
      s.loan_payment,s.driver_net result FROM company_trip_settlements s JOIN trip_settlement_completions sc ON sc.trip_id=s.trip_id AND sc.user_id=s.user_id JOIN trips t ON t.id=s.trip_id
  ...
  app.get('/me/ranking',async c=>{
    ...
    try{
      ...
      if(companyId){
        if(since===null){
          rows=await sql`WITH members AS (SELECT u.id,u.name,u.email,cm.registration_number FROM company_members cm JOIN users u ON u.id=cm.user_id WHERE cm.company_id=${companyId} AND cm.status='active' AND u.status='active'), incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT m.id,m.name,m.email,m.registration_number,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM members m LEFT JOIN trips t ON t.user_id=m.id AND t.status='finished' AND EXISTS (SELECT 1 FROM trip_settlement_completions sc WHERE sc.trip_id=t.id AND sc.user_id=t.user_id) LEFT JOIN incomes i ON i.trip_id=t.id GROUP BY m.id,m.name,m.email,m.registration_number`;
        }else{
          rows=await sql`WITH members AS (SELECT u.id,u.name,u.email,cm.registration_number FROM company_members cm JOIN users u ON u.id=cm.user_id WHERE cm.company_id=${companyId} AND cm.status='active' AND u.status='active'), incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT m.id,m.name,m.email,m.registration_number,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM members m LEFT JOIN trips t ON t.user_id=m.id AND t.status='finished' AND EXISTS (SELECT 1 FROM trip_settlement_completions sc WHERE sc.trip_id=t.id AND sc.user_id=t.user_id) AND COALESCE(t.finished_at,t.started_at)>=${since.toISOString()} LEFT JOIN incomes i ON i.trip_id=t.id GROUP BY m.id,m.name,m.email,m.registration_number`;
        }
      }else{
        if(since===null){
          rows=await sql`WITH incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT u.id,u.name,u.email,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM users u LEFT JOIN trips t ON t.user_id=u.id AND t.status='finished' AND EXISTS (SELECT 1 FROM trip_settlement_completions sc WHERE sc.trip_id=t.id AND sc.user_id=t.user_id) LEFT JOIN incomes i ON i.trip_id=t.id WHERE u.id=${u.id} GROUP BY u.id,u.name,u.email`;
        }else{
          rows=await sql`WITH incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT u.id,u.name,u.email,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM users u LEFT JOIN trips t ON t.user_id=u.id AND t.status='finished' AND EXISTS (SELECT 1 FROM trip_settlement_completions sc WHERE sc.trip_id=t.id AND sc.user_id=t.user_id) AND COALESCE(t.finished_at,t.started_at)>=${since.toISOString()} LEFT JOIN incomes i ON i.trip_id=t.id WHERE u.id=${u.id} GROUP BY u.id,u.name,u.email`;
        }
      }
      const ranking=rows.map((r:any)=>{const km=Number(r.km||0),revenue=Number(r.revenue_brl||0),trips=Number(r.trips||0);return {id:r.id,name:r.name,email:r.email,registrationNumber:r.registration_number??null,trips,km:Number(km.toFixed(1)),revenueBrl:Number(revenue.toFixed(2)),rateBrlKm:km>0?Number((revenue/km).toFixed(2)):0}});
