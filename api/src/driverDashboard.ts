...
export function registerDriverDashboardRoutes(app:any){
  ...
  app.get('/me/ranking',async c=>{
    ...
    try{
      ...
      if(companyId){
        if(since===null){
          rows=await sql`WITH members AS (SELECT u.id,u.name,u.email,cm.registration_number FROM company_members cm JOIN users u ON u.id=cm.user_id WHERE cm.company_id=${companyId} AND cm.status='active' AND u.status='active'), incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT m.id,m.name,m.email,m.registration_number,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM members m LEFT JOIN trips t ON t.user_id=m.id AND t.status='finished' LEFT JOIN incomes i ON i.trip_id=t.id GROUP BY m.id,m.name,m.email,m.registration_number`;
        }else{
          rows=await sql`WITH members AS (SELECT u.id,u.name,u.email,cm.registration_number FROM company_members cm JOIN users u ON u.id=cm.user_id WHERE cm.company_id=${companyId} AND cm.status='active' AND u.status='active'), incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT m.id,m.name,m.email,m.registration_number,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM members m LEFT JOIN trips t ON t.user_id=m.id AND t.status='finished' AND COALESCE(t.finished_at,t.started_at)>=${since.toISOString()} LEFT JOIN incomes i ON i.trip_id=t.id GROUP BY m.id,m.name,m.email,m.registration_number`;
        }
      }else{
        if(since===null){
          rows=await sql`WITH incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT u.id,u.name,u.email,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM users u LEFT JOIN trips t ON t.user_id=u.id AND t.status='finished' LEFT JOIN incomes i ON i.trip_id=t.id WHERE u.id=${u.id} GROUP BY u.id,u.name,u.email`;
        }else{
          rows=await sql`WITH incomes AS (SELECT l.trip_id,MAX(l.amount_brl)::numeric AS gross_brl FROM economy_ledger l WHERE l.entry_type='trip_income' AND l.amount_brl>0 GROUP BY l.trip_id) SELECT u.id,u.name,u.email,COUNT(t.id)::int AS trips,COALESCE(SUM(t.distance_km),0)::numeric AS km,COALESCE(SUM(COALESCE(i.gross_brl,0)),0)::numeric AS revenue_brl FROM users u LEFT JOIN trips t ON t.user_id=u.id AND t.status='finished' AND COALESCE(t.finished_at,t.started_at)>=${since.toISOString()} LEFT JOIN incomes i ON i.trip_id=t.id WHERE u.id=${u.id} GROUP BY u.id,u.name,u.email`;
        }
      }
