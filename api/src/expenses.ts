...
export function registerExpenseRoutes(app:any){
 ...
 app.post('/me/expenses/fuel-payment',async c=>{try{
   ...
   if(!station||station.length>160)return jsonError('Posto inválido.',400);if(!city||city.length>120)return jsonError('Cidade inválida.',400)
   ...
   const description=`Abastecimento • ${liters.toFixed(1)} L x R$ ${price.toFixed(2)}/L • ${station} • ${city}`
