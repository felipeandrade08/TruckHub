...
public partial class MainWindow
{
    ...
    private async Task TryRecoverActiveTrip()
    {
        ...
        try
        {
            ...
            foreach (var trip in trips.EnumerateArray())
            {
                ...
                if (!TripMatchesTelemetry(trip, data)) continue;
                activeTrip = trip;
                break;
            }
            ...
            var tripId = idElement.GetString();
            ...
            TripStatusText.Text = "VIAGEM RECUPERADA AUTOMATICAMENTE";
    ...
    private async Task RestoreTripBaseline(string tripId, string token, TelemetrySnapshot current)
    {
        try
        {
            using var request = new HttpRequestMessage(HttpMethod.Get, $"{ApiBaseUrl}/me/tachographs/{tripId}/samples");
            request.Headers.TryAddWithoutValidation("Cookie", $"truckhub_session={token}");
            using var response = await _http.SendAsync(request);
            if (!response.IsSuccessStatusCode) return;

            using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
            if (!document.RootElement.TryGetProperty("samples", out var samples) || samples.ValueKind != JsonValueKind.Array || samples.GetArrayLength() == 0) return;
            var first = samples[0];
            if (first.TryGetProperty("odometer_km", out var odometer) && odometer.TryGetSingle(out var startOdometer))
                _tripStartOdometer = startOdometer;
            if (first.TryGetProperty("fuel_l", out var fuel) && fuel.TryGetSingle(out var startFuel))
                _tripStartFuel = startFuel;
        }
        catch
        {
            _tripStartOdometer = current.OdometerKm;
            _tripStartFuel = current.FuelLiters;
        }
    }

    private static bool IsOpenTrip(JsonElement trip)
    {
        if (trip.TryGetProperty("finished_at", out var finished) && finished.ValueKind != JsonValueKind.Null && finished.ValueKind != JsonValueKind.Undefined)
            return false;
        if (trip.TryGetProperty("status", out var status))
        {
            var value = status.GetString();
            if (!string.IsNullOrWhiteSpace(value) && !value.Equals("active", StringComparison.OrdinalIgnoreCase)) return false;
        }
        return true;
    }
    ...
    {
        var cargo = ReadString(trip, "cargo");
        var destination = ReadString(trip, "destination");
        var origin = ReadString(trip, "origin");
    ...
    private static DateTime? ReadDateTime(JsonElement element, string property)
    {
        ...
        return DateTime.TryParse(value, out var result) ? result.ToUniversalTime() : null;
    }
