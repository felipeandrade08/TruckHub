...
public partial class MainWindow
{
    ...
    private async Task PollOperationalTelemetry()
    {
        try
        {
            using var response = await _opsHttp.GetAsync(TelemetryUrl);
    ...
    private void ResetFuelingCandidate()
    {
    ...
    {
        // O aviso de bloqueio da garagem não pode ser sobrescrito pelo
        // alerta de autonomia: eles disputavam o mesmo TextBlock.
        if (_garageUnauthorized)
        {
            FuelAutoText.Text = $"Abastecimento automático: monitorando • {data.FuelLiters:0.0} L";
            return;
        }
        ...
        {
            AlertText.Text = "⛽ ABASTECIMENTO DETECTADO • aguardando estabilização";
