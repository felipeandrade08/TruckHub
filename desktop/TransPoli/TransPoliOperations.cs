...
public partial class MainWindow
{
    ...
    private void LoadOperations(){try{if(!File.Exists(_operationsPath))return;var state=JsonSerializer.Deserialize<OperationsState>(File.ReadAllText(_operationsPath));if(state is null)return;_refuelings.AddRange(state.Refuelings??new());_nextRefuelingNumber=Math.Max(state.NextRefuelingNumber,_refuelings.Count==0?1:_refuelings.Max(x=>x.Number)+1);_stops.AddRange(state.Stops??new());_occurrences.AddRange(state.Occurrences??new());_documents.AddRange(state.Documents??new());_poliPassRecords.AddRange(state.PoliPassRecords??new());}catch{}}
