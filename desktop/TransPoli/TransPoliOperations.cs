...
public partial class MainWindow
{
    ...
    private void LoadOperations(){try{if(!File.Exists(_operationsPath))return;var state=JsonSerializer.Deserialize<OperationsState>(File.ReadAllText(_operationsPath));if(state is null)return;_refuelings.AddRange(state.Refuelings??new());
        ...
        _stops.AddRange(state.Stops??new());_occurrences.AddRange(state.Occurrences??new());_documents.AddRange(state.Documents??new());_poliPassRecords.AddRange((state.PoliPassRecords??new()).GroupBy(x=>x.EventId).Select(g=>g.OrderBy(x=>x.RecordedAtUtc).First()));SaveOperations();}catch{}}
