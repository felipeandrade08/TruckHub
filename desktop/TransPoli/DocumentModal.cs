...
public partial class MainWindow
{
    ...
    private static void InterceptDocumentButton(object sender, RoutedEventArgs e)
    {
        ...
        if (button.DataContext is MainWindow) return;
        if (button.TemplatedParent is not null) return;
        if (button.IsDescendantOf((DependencyObject)button)) return;
    ...
    private void ShowDocumentsInsideTablet()
    {
        ...
        {
            Text = "Gerencie documentos sem abrir outra janela ou aba.",
            FontSize = 11,
        ...
        var status = new ComboBox
        {
            ...
            ItemsSource = new[] { "Aguardando nota", "Recebido", "Conferido", "Carimbado", "Despachado" },
            SelectedIndex = 0,
        ...
        foreach (var item in _documents.OrderByDescending(x => x.RecordedAtUtc).Take(20))
            ...
            {
                Text = $"{item.RecordedAtUtc.ToLocalTime():dd/MM HH:mm} • {item.Status}\n{item.Reference}",
                FontSize = 11,
                Foreground = FindResource("Text") as Brush,
        ...
        save.Click += (_, _) =>
        {
            ...
            SaveOperations();
            UpdateOpsCounters();
            StatusText.Text = $"TransPoli • documento atualizado • {selected}";
