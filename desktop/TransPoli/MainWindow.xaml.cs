...
public partial class MainWindow : Window
{
    ...
    private void DriverPhone_InvoiceViewRequested(string reference)
    {
        ...
        private void ShowPoliPassReceipt(PoliPassRecord record)
    {
        ...
        var muted = new SolidColorBrush(Color.FromRgb(88, 94, 101));
        var paid = record.Amount > 0;
        var localAt = record.RecordedAtUtc.ToLocalTime();
        var receiptNo = $"PP-{Math.Max(0, record.EventId):0000000000}";
        var truck = $"{record.TruckBrand} {record.TruckModel}".Trim();
        if (string.IsNullOrWhiteSpace(truck)) truck = "VEÍCULO NÃO INFORMADO";
        var trailerCount = record.Trailers?.Count ?? 0;

        TextBlock T(string value,double size=13,FontWeight? weight=null,Brush? color=null) => new()
        {
            Text=value,FontSize=size,FontWeight=weight??FontWeights.Normal,Foreground=color??Brushes.Black,
            TextWrapping=TextWrapping.Wrap,VerticalAlignment=VerticalAlignment.Center
        };
        UIElement Row(string label,string value)
        {
            ...
            var l=T(label,11,FontWeights.SemiBold,muted);var v=T(value,14,FontWeights.Bold,Brushes.Black);
            Grid.SetColumn(v,1);g.Children.Add(l);g.Children.Add(v);return g;
        }

        var paper=new Border{Background=Brushes.White,CornerRadius=new CornerRadius(16),Padding=new Thickness(0),MaxWidth=1080};
        var body=new StackPanel();

        var header=new Grid{Background=dark,Margin=new Thickness(0),Height=150};
        header.ColumnDefinitions.Add(new ColumnDefinition{Width=new GridLength(1,GridUnitType.Star)});
        header.ColumnDefinitions.Add(new ColumnDefinition{Width=new GridLength(360)});
        var brand=new StackPanel{Margin=new Thickness(34,24,20,18)};
        brand.Children.Add(T("POLIPASS",42,FontWeights.ExtraBold,gold));
        brand.Children.Add(T("T R A N S P O L I",14,FontWeights.SemiBold,Brushes.White));
        brand.Children.Add(T("PEDÁGIOS  •  MOBILIDADE  •  SERVIÇOS",10,FontWeights.Medium,new SolidColorBrush(Color.FromRgb(205,210,214))));
        header.Children.Add(brand);
        var hn=new StackPanel{Margin=new Thickness(20,30,30,18),HorizontalAlignment=HorizontalAlignment.Right};
        hn.Children.Add(T("COMPROVANTE DE PASSAGEM",13,FontWeights.Bold,Brushes.White));
        hn.Children.Add(T(receiptNo,24,FontWeights.ExtraBold,gold));
        hn.Children.Add(T(paid?"✓ PAGAMENTO CONFIRMADO":"PAGAMENTO PENDENTE",11,FontWeights.Bold,paid?new SolidColorBrush(Color.FromRgb(78,229,155)):gold));
        Grid.SetColumn(hn,1);header.Children.Add(hn);body.Children.Add(header);

        var title=new Grid{Margin=new Thickness(28,22,28,8)};
        title.ColumnDefinitions.Add(new ColumnDefinition{Width=new GridLength(1,GridUnitType.Star)});
        title.ColumnDefinitions.Add(new ColumnDefinition{Width=GridLength.Auto});
        title.Children.Add(T("COMPROVANTE DE PASSAGEM\nPEDÁGIO ELETRÔNICO POLIPASS",24,FontWeights.ExtraBold,Brushes.Black));
        var status=T(paid?"PAGO":"PENDENTE",18,FontWeights.ExtraBold,paid?new SolidColorBrush(Color.FromRgb(20,145,92)):gold);
        Grid.SetColumn(status,1);title.Children.Add(status);body.Children.Add(title);

        var details=new StackPanel{Margin=new Thickness(34,8,34,14)};
        details.Children.Add(Row("DATA / HORA",localAt.ToString("dd/MM/yyyy HH:mm:ss")));
        details.Children.Add(Row("VEÍCULO",truck));
        details.Children.Add(Row("PLACA",string.IsNullOrWhiteSpace(record.LicensePlate)?"NÃO INFORMADA":record.LicensePlate));
        details.Children.Add(Row("EIXOS DO CONJUNTO",record.TotalAxles.HasValue?$"{record.TotalAxles.Value} eixos":"NÃO CONFIRMADOS"));
        details.Children.Add(Row("REBOQUES",trailerCount>0?$"{trailerCount} detectado(s)":"NENHUM / NÃO INFORMADO"));
        details.Children.Add(Row("PESO DA CARGA",record.CargoMassKg>0?$"{record.CargoMassKg/1000.0:0.0} t":"NÃO INFORMADO"));
        details.Children.Add(Row("TIPO DE OPERAÇÃO","PASSAGEM AUTOMÁTICA POLIPASS"));
        details.Children.Add(Row("PRAÇA / RODOVIA","NÃO INFORMADO PELA TELEMETRIA ETS2"));
        body.Children.Add(details);

        var amountBar=new Grid{Background=dark,Margin=new Thickness(18,4,18,14),Height=116};
        amountBar.ColumnDefinitions.Add(new ColumnDefinition{Width=new GridLength(1,GridUnitType.Star)});
        amountBar.ColumnDefinitions.Add(new ColumnDefinition{Width=new GridLength(350)});
        var msg=new StackPanel{Margin=new Thickness(28,20,20,15)};
        msg.Children.Add(T("POLIPASS TRANSPOLI",19,FontWeights.ExtraBold,gold));
        msg.Children.Add(T("Passagem registrada pela telemetria e confirmada pelo Banco TransPoli.",11,FontWeights.Normal,Brushes.White));
        amountBar.Children.Add(msg);
        var val=new StackPanel{Margin=new Thickness(20,16,28,12),HorizontalAlignment=HorizontalAlignment.Right};
        val.Children.Add(T("VALOR DA PASSAGEM",12,FontWeights.Bold,Brushes.White));
        val.Children.Add(T(paid?record.Amount.ToString("C2",CultureInfo.GetCultureInfo("pt-BR")):"AGUARDANDO",34,FontWeights.ExtraBold,gold));
        Grid.SetColumn(val,1);amountBar.Children.Add(val);body.Children.Add(amountBar);

        body.Children.Add(new TextBlock
        {
            Text="Evento detectado pela telemetria ETS2. Comprovante operacional TransPoli / PoliPass, sem validade fiscal. Campos não fornecidos pela telemetria permanecem como não informados.",
            FontSize=10,Foreground=muted,Margin=new Thickness(28,4,28,22),TextWrapping=TextWrapping.Wrap
        });
        paper.Child=body;

        var scroll=new ScrollViewer{Content=paper,VerticalScrollBarVisibility=ScrollBarVisibility.Auto,HorizontalScrollBarVisibility=ScrollBarVisibility.Auto};
        ShowModalContent("polipass-receipt",BuildModalCard("POLIPASS • COMPROVANTE",scroll,paid?"Pagamento confirmado • Banco TransPoli":"Pagamento ainda não confirmado"));
    }
