...
public partial class MainWindow
{
    internal async void ShowRealisticInvoiceModal()
    {
        ...
        _documentModalKind = "invoice-v109";
        ...
        try
        {
            ...
            foreach (var trip in doc.RootElement.GetProperty("trips").EnumerateArray()) { selected = trip; break; }
            _documentModalLayer.Child = BuildRealisticInvoice(selected);
        }
        catch
        {
            _documentModalLayer.Child = new Border { Background = Brushes.White, Padding = new Thickness(30), CornerRadius = new CornerRadius(20), Child = new TextBlock { Text = "Não foi possível gerar a nota agora.", Foreground = Brushes.Black, FontSize = 16 } };
        }
    }
    ...
    private UIElement BuildRealisticInvoice(JsonElement? trip)
    {
        ...
        brand.Children.Add(new Image { Source = new BitmapImage(new Uri("pack://application:,,,/TransPoli;component/Assets/transpoli-logo.png")), Width = 70, Height = 70, Margin = new Thickness(0,0,14,0) });
        ...
        root.Children.Add(new Border { BorderBrush = Brushes.Black, BorderThickness = new Thickness(0,1,0,1), Margin = new Thickness(0,18,0,12), Padding = new Thickness(0,8), Child = new TextBlock { Text = $"EMISSÃO: {DateTime.Now:dd/MM/yyyy HH:mm:ss}    SÉRIE: TP-01", Foreground = Brushes.Black, FontSize = 10 } });
        ...
        root.Children.Add(InvoiceSection("VALORES", $"Valor da carga: {FormatBrl(value)}\nFrete calculado pelo TruckHub: {FormatBrl(value)}"));
