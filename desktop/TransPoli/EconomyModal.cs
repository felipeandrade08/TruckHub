...
public partial class MainWindow
{
    ...
    internal async void ShowEconomyModal()
    {
        ...
        try
        {
            ...
            var data = await LoadEconomyDataAsync(token);
            _documentModalLayer.Child = BuildEconomyContent(data);
        }
        catch
        {
    ...
    private async Task<EconomyViewData> LoadEconomyDataAsync(string token)
    {
        ...
        if (rates.IsSuccessStatusCode)
        {
            ...
            foreach (var row in ratesDoc.RootElement.GetProperty("rates").EnumerateArray())
                result.Rates.Add($"{row.GetProperty("display_name").GetString()}: {FormatBrl(row.GetProperty("rate_brl_km").GetDecimal())}/km");
        }
    ...
    private UIElement BuildEconomyLoading() => new Border
    {
        Background = FindResource("Bg") as Brush,
        CornerRadius = new CornerRadius(24),
        Padding = new Thickness(30),
        Child = new TextBlock { Text = "💰 CARREGANDO BANCO DO MOTORISTA...", Foreground = FindResource("Text") as Brush, FontSize = 20, FontWeight = FontWeights.Bold, HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center }
    };

    private UIElement BuildEconomyError(string message) => new Border
    {
        Background = FindResource("Bg") as Brush,
        CornerRadius = new CornerRadius(24),
        Padding = new Thickness(30),
        Child = new StackPanel { Children = { new TextBlock { Text = "💰 BANCO DO MOTORISTA", Foreground = FindResource("Text") as Brush, FontSize = 22, FontWeight = FontWeights.Bold }, new TextBlock { Text = message, Foreground = FindResource("Muted") as Brush, FontSize = 14, TextWrapping = TextWrapping.Wrap, Margin = new Thickness(0, 14, 0, 0) } } }
    };
    ...
    private UIElement BuildEconomyContent(EconomyViewData data)
    {
        ...
        {
            panel.Children.Add(ModalLine($"Empréstimo: {FormatBrl(data.LoanPrincipal)}\nSaldo devedor: {FormatBrl(data.LoanRemaining)}\nDesconto automático: {data.LoanPct:0.##}% da receita líquida", 13));
        }
        else
        {
    ...
    private async Task RequestLoanAsync(decimal amount)
    {
        ...
        StatusText.Text = response.IsSuccessStatusCode ? $"TransPoli • empréstimo de {FormatBrl(amount)} liberado" : "TransPoli • não foi possível liberar o empréstimo";
        ShowEconomyModal();
    }
